...
public class Node implements java.io.Serializable, Anchor, Siteable, PageUrlResolver {
	...
	@Transient
	public boolean isLinked() {//如果它返回真，geturl就调用Dynamic
	...
	public String getTemplate() {
		...
		if (StringUtils.isBlank(tpl)) {
			...
			if (getParent() == null) {
				tpl = getTemplate(model.getCustoms());//根节点
			} else {
				if (CollectionUtils.isEmpty(getChildren())) {//叶子节点
	...
	public NodeDetail getDetail() {
		...
		if (!CollectionUtils.isEmpty(set)) {//如果走这里，就不是空，就调用Dynamic
	...
	public Set<NodeDetail> getDetails() {//details有值就调用动态url
		return details;
	}
